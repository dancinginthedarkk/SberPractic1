...
const useThreeScene = (containerRef, loaderRef, initialPanoramaId = 1) => {
  let camera, scene;
  let arrowMeshes = [];
  let textMeshes = [];
  let isUserInteracting = false,
    ...
    theta = 0;
  let panoramaMeshes = [];
  let currentPanoramaIndex = config.findIndex(
    (panorama) => panorama.id === initialPanoramaId
  );

  const [currentPanoramaId, setCurrentPanoramaId] = useState(
    initialPanoramaId
  );
...
renderer.gammaFactor = 2.2;
  ...
  const init = () => {
    ...
    if (config[currentPanoramaIndex].arrows) {
      config[currentPanoramaIndex].arrows.forEach((arrow) => {
        ...
        loader.load('/fonts/Arrow.STL', function (geometry) {
          ...
          loader.load('/fonts/Roboto-Regular.ttf', function (ttf) {
    ...
    renderer.domElement.addEventListener('click', (event) => {
      ...
      mouseClickPositionVector2D.x =
        (x / renderer.domElement.clientWidth) * 2 - 1;
      mouseClickPositionVector2D.y =
        ...
        gsap.to(camera.position, {
          ...
          onComplete: () => {
            currentPanoramaIndex = config.findIndex(
              (panorama) =>
                panorama.id === intersects[0].object.userData.panoramaId
            );
